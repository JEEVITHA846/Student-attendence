    ...
    <div className="animate-in fade-in duration-500 space-y-10">
      ...
      {selectedFolder && !selectedSession && (
        <div className="bg-white rounded-[2.5rem] border border-slate-50 shadow-sm overflow-hidden animate-in slide-in-from-bottom-4 duration-300 mb-24">
          <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
             <button onClick={handleBack} className="flex items-center gap-2 text-slate-500 hover:text-blue-600 transition-colors font-black text-xs uppercase tracking-widest">
                  ...
                  {Object.keys(dateFolders[selectedFolder].reduce((acc: any, curr) => {
                    ...
                    <div key={time} className="relative group">
                      <button 
                        ...
                        className="w-full flex items-center justify-between p-6 bg-slate-50 rounded-[2rem] group hover:bg-white hover:shadow-xl transition-all border border-transparent hover:border-slate-100 text-left"
                      >
                      ...
                      <button 
                        ...
                        className="absolute right-14 top-1/2 -translate-y-1/2 p-2 text-slate-200 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
      ...
      {selectedFolder && selectedSession && detailData && (
        <div className="space-y-10 animate-in slide-in-from-right-4 duration-300 pb-24">
          <div className="flex items-center justify-between bg-white p-6 rounded-[2rem] border border-slate-50 shadow-sm">
             <button onClick={handleBack} className="flex items-center gap-2 text-slate-500 hover:text-blue-600 transition-colors font-black text-xs uppercase tracking-widest">
                  <ArrowLeft size={18} /> Back
             </button>
             <div className="flex items-center gap-3">
                <Clock size={16} className="text-blue-500" />
                <h2 className="text-sm font-black text-slate-900">{selectedSession}</h2>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
             {/* Lists - Reused standardized logic */}
             {[
                    ...
                    {col.data.map(rec => {
                      ...
                      return (
                        <div key={rec.id} className={`p-5 ${col.bg} rounded-2xl border border-transparent`}>
                          ...
                          <p className="text-xs font-bold text-slate-400 uppercase mt-1.5 tracking-wider">{student?.department}</p>
                        </div>
                      );
                    })}
